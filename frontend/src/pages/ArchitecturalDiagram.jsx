...
export default function ArchitecturalDiagram() {
  ...
  const renderAllDiagrams = () => {
   ...
   graph TD
    %% Add invisible spacer nodes for better layout
    FrontendLayerStart([ ]):::spacer
    FrontendLayerEnd([ ]):::spacer

    subgraph FrontendLayer["Frontend Layer - React-Vite"]
        FrontendLayerStart --> F1[User Interface]
        F1 --> F2[State Management<br/>React Query / Redux]
    ...
    style FrontendLayer fill:#f0f9ff,stroke:#0366d6,stroke-width:2px,rx:10,ry:16
    `);
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center justify-between px-4 py-2 border rounded-lg text-sm font-semibold shadow-sm transition-all ${
              expanded[key]
                ? "bg-gradient-to-r from-gray-200 via-blue-200 to-gray-200 text-gray-600"
